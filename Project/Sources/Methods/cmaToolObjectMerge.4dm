//%attributes = {}
/*------------------------------------------------------------------------------
Méthode : outilsObjectMerge (composant CioWeb)
	...
	Case of 
		...
		: (OB Get type:C1230($oFils; $key_t)=Is object:K8:27) & (OB Get type:C1230($oParent; $key_t)=Is object:K8:27)
			...
			Else   // Du moment qu'ils ont le même type, on prend la valeur du fils 
				$oFusion[$key_t]:=cmaToolObjectMerge($oParent[$key_t]; $oFils[$key_t])
			End if 
				...
				If ($KeyIdName_t#"")  // On commence par dupliquer le parent dans la fusion.
					...
					For each ($objectCollectionSon_o; $oFils[$key_t])  // On récupére la valeur de ID du fils.
						...
						If ($indicesParent_c.length=1)
							$oFusion[$key_t][$indicesParent_c[0]]:=cmaToolObjectMerge($oParent[$key_t][$indicesParent_c[0]]; $objectCollectionSon_o)
						Else 
